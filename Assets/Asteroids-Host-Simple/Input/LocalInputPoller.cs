...
{
    private UISpaceShipInput _uiInput;
    ...
    {
        if (_uiInput == null) _uiInput = FindObjectOfType<UISpaceShipInput>();
        if (_uiInput == null) return;

            SpaceshipInput localInput = new SpaceshipInput();

        localInput.HorizontalInput = _uiInput.Horizontal;
        localInput.VerticalInput = _uiInput.Vertical;
        localInput.Buttons.Set(SpaceshipButtons.Fire, _uiInput.NeedFire);
